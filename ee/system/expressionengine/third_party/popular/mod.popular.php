...
class Popular {
    ...
    */
    ...
    public function entry_history()
    {
        ...
        if (ee()->extensions->active_hook('popular_entry_history_end') === TRUE)
		{
			ee()->extensions->call('popular_entry_historys_end',$tagdata,$variables);
			if (ee()->extensions->end_script === TRUE) return;
			if (ee()->extensions->last_call  !== FALSE) return ee()->extensions->last_call;
		}
    ...
    */
    ...
    public function entry_views_by_interval()
    {
        $tagparams = ee()->TMPL->tagparams;
        $tagdata   = ee()->TMPL->tagdata;

        if (ee()->extensions->active_hook('popular_entry_views_by_interval_start') === TRUE)
		{
			ee()->extensions->call('popular_entry_views_by_interval_start',$tagparams,$tagdata);
			if (ee()->extensions->end_script === TRUE) return;
		}
        ...
        $order_by          = ee()->TMPL->fetch_param('order_by','view_date');
        $sort              = ee()->TMPL->fetch_param('sort','desc');
        $interval          = ee()->TMPL->fetch_param('interval','day');
        $adjust_month      = ee()->TMPL->fetch_param('adjust_month','+ 0');
        $backspace         = ee()->TMPL->fetch_param('backspace',0);
               
        $cache             = ee()->TMPL->fetch_param('cache','no');
        $refresh           = ee()->TMPL->fetch_param('refresh','60');
        
        $cache_key = base64_encode(serialize($tagparams)) . 'views_by_interval'; 
        ...
        $this->set_results($limit,'',$start_on,$stop_before,$user_device,$device_type,$user_country_code,$country_code,$user_region_code,$region_code,$user_city,$city,$order_by,$sort,'',$entry_id,$url_title,FALSE);
        ...
        foreach( $this->entry_pool->result_array() as $view )
	    {
	        
	        $year   = substr($view['view_date'], 0,4);
            $month  = substr($view['view_date'], 5,2);
            $date   = substr($view['view_date'], 8,2);
            $hour   = substr($view['view_date'], 11,2);
            
            switch($interval)
            {
                case 'hour':
                    $date_key = $year.'-'.$month.'-'.$date.'-'.$hour;
                    break;
                case 'month':
                    $date_key = $year.'-'.$month;
                    break;
                case 'year':
                    ...
                    break;
                default:
                    $date_key = $year.'-'.$month.'-'.$date;
                    break;
            }
            
            if( array_key_exists($date_key, $activity) ){
                $activity[$date_key]['total'] += 1;
            } else {
                $activity[$date_key]['total'] = 1;
            }
	    ...
	    foreach( $activity as $date => $hits )
	    {
            ...
            $month  = (int) substr($date, 5,2) + (int) $adjust_month;
            ...
            $variables[] = array(
                ...
                $prefix . 'year'         => $year, 
                $prefix . 'hour'         => $hour,    
                $prefix . 'total_hits'   => $total_hits              
            );    
	    }

        
        if (ee()->extensions->active_hook('popular_entry_views_by_interval_end') === TRUE)
		{
			ee()->extensions->call('popular_entry_views_by_interval_end',$tagdata,$variables);
			if (ee()->extensions->end_script === TRUE) return;
			if (ee()->extensions->last_call  !== FALSE) return ee()->extensions->last_call;
		}
		
		$output = trim(ee()->TMPL->parse_variables(ee()->TMPL->tagdata, $variables));
